...
|| =clatter-display-on-join=            | =nil=           | Show the channel buffer when you join                            |
|| =clatter-display-on-welcome=         | =nil=           | Show the server buffer after the welcome numeric                 |
...
| =clatter-track-layout=                  | ='legacy=          | =legacy= output or opt-in adaptive =strip=                             |
| =clatter-track-max-width=               | =0.4=              | Strip allocation as window fraction or integer columns                   |
| =clatter-track-show-counts=             | =t=                | Show unread counts                                                       |
| =clatter-track-count-style=             | ='suffix=          | =suffix=, =parens=, =superscript=, =subscript=, =glyph=, =none=          |
...
(=#system-crafters= becomes =#sc=). With =nil=, the legacy layout shows the
full =clatter:network/#channel= buffer name and the strip layout shows the raw
target. Colliding strip labels fall back to raw targets and then gain a network
or buffer identity suffix. Query nicks and the server buffer are never
shortened. Shortening applies only to the compact mode-line indicator;
=M-x clatter-track-list= always shows the full =clatter:network/#channel=
buffer name. Hovering an entry (in the mode line or =*clatter-activity*=)
shows a tooltip with the full, unshortened channel name and unread count.

The default =legacy= layout preserves the existing indicator. To opt into the
width-aware strip and parenthesized counts:

#+begin_src emacs-lisp
(setopt clatter-track-layout 'strip
        clatter-track-count-style 'parens
        clatter-track-max-width 0.4)
(custom-set-faces
 '(clatter-track-count ((t (:height 0.9)))))
#+end_src

The strip uses =clatter-track-max-width= as a fraction of its window or, for an
integer, a column budget. Zero hides it. Customize =clatter-track-count= with
native face attributes such as relative =:height=; terminals ignore font size.
...
=M-x clatter-chathistory-targets= queries every connected server and displays
its channels and query targets as a tree.  It uses the activity-list navigation
keys: =RET= visits, =o= visits in another window, =a= visits and buries the
browser, and =g= refreshes.
