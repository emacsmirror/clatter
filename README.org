...
A dedicated, fully IRCv3-compliant IRC client for Emacs. Pure Elisp. No external dependencies beyond Emacs itself.
...
- =SPC i o= to browse all URLs from the buffer (completing-read picker)
- =SPC i y= to copy a URL from the buffer to kill ring
...
- =SPC i t= to jump to most urgent activity
- =SPC i l= to list all activity
...
- =SPC i n= to toggle, =SPC i k= to add keywords
...
clatter.el has no external dependencies, but works well with:
